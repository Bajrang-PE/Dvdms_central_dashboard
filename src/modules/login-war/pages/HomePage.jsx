...
const HomePage = () => {
    const { showCmsLogin, setShowCmsLogin, showForgotPass, setShowForgotPass, homeItemCounts } = useContext(LoginContext);
        ...
        <div className="main-portal-bg">
            ...
            <div className='container py-4' style={{ maxWidth: "100%", paddingLeft: "30px", paddingRight: "30px" }}>
                ...
                <div className="top-counter-ribbon mb-4">
                    {homeItemCounts?.length > 0 && homeItemCounts?.filter(dt => dt?.kpiResponse?.name)?.map((item, index) => (
                        <div className="counter-item" key={index}>
                            <div className="counter-icon"><i className="fas fa-laptop-code"></i></div>
                            <div className="counter-info">
                                <h3>{item?.kpiResponse?.count ?? 0} </h3>
                                <p>{item?.kpiResponse?.name || "NA"}</p>
                            </div>
                        </div>
                    ))}
                </div>
